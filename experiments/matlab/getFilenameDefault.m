...
% separated by delim.
    ...
    p.addParameter('exclude', {'is_learned', 'is_unlearned'});
    ...
    for i = 1:length(fields)
        if ~any(cellfun(@(x) strcmp(x, fields{i}), p.Results.exclude))
            filename = strcat(filename, p.Results.delim, string(stim.(fields{i})));
        end
    end
    
    filename = char(strcat(filename, p.Results.extension));
end