...
if contains(dataFolder, 'flower', 'IgnoreCase', true)
    ...
    featVStr = '%d,%s,%s,%s,%d,';
    fprintf(catlog,'subject,experimentNumber,task,condition,featureLearned,valueLearned,featureFoil,valueFoil,numPetal,petalColor,centerShape,centerColor,numSepal,isTarget,isFoil,wasCorrect,RT,isOld\n');
elseif contains(dataFolder, 'insect','IgnoreCase',true)
    ...
    fprintf(catlog,'subject,experimentNumber,task,condition,featureLearned,valueLearned,featureFoil,valueFoil,segmentCount,segmentShape,wingColor,antennaeCount,antennaeColor,isTarget,isFoil,wasCorrect,RT,isOld\n');
elseif contains(dataFolder, 'turtle','IgnoreCase',true)
    featVStr = '%d,%s,%s,%s,%s,';
    featureSet = {'numSpots', 'spotColor', 'legShape', 'legColor', 'shellShape'};
    fprintf(catlog,'subject,experimentNumber,task,condition,featureLearned,valueLearned,featureFoil,valueFoil,spotCount,spotColor,legShape,legColor,shellShape,isTarget,isFoil,wasCorrect,RT,isOld\n');
end
...
for sbj = 1:numSub
    ...
    for tr = 1:totTrs
        ...
        fprintf(catlog,['%d,%d,%s,%d,%s,%s,%s,%s,',featVStr,'%d,%d,%d,%d,%d\n'],...
            sbj,catS.experimentNumber,taskStr,catS.condition,...
